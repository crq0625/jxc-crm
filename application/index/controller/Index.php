...
class Index extends Common_base
{
    public function index()
    {
        ...
        $user = Db::name('users')->alias('u')
            ...
            ->find();
        $pcount = Db::name('purchase')
            ->where('status', '>=', 0)
            ->count();//共计订单

        $pshenc = Db::name('purchase')
            ->where('status', '=', 2)
            ->count();//生产订单

        $pchuhuo = Db::name('purchase')
            ->where('status', '=', 5)
            ->count();//完成订单

        $pqueren = Db::name('purchase')
            ->where('affirm', '=', 0)
            ->where('status', '=',0)
            ->count();//待确认订单

        $pqrdjin = Db::name('purchase')
            ->where('affirm', '=',1)
            ->where('status', '=',0)
            ->count();//待确认订金

        $pqrweikuan = Db::name('purchase')
            ->where('affirm', '=', 1)
            ->where('status', '=',3)
            ->count();//待确认尾款

        $pqrchuku = Db::name('purchase')
            ->where('affirm', '=', 1)
            ->where('status', '=',4)
            ->count();//待确认出库
